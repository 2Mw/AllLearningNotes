...
其中快速幂的方法就是将 $a^k$ 分解为 $a^k=a^{2^{i_{x_1}}} + a^{2^{i_{x_2}}} + \cdots+a^{2^n}$，其中 $k$ 的二进制可以看作 $(i_1i_2...i_k)_2$，并且 $i_{x_j}=1$。
...
* **线性筛选法**
  ...
  这里需要注意几个点：在进行乘法的时候会出现超出 int 范围的情况，因此需要将 $primes[j] * i \leq n$ 替换为 $primes[j]\leq n/ i$ 。
