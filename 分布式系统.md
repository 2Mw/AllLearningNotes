...
由于读写请求都是发送给 Raft Leader，如果请求量十分巨大，Leader 可能会达到性能极限，导致性能瓶颈。
...
type PubSub interface {
    ...
    Subscribe(c char<- Event)
    
    Cancel(c char<- Event)
}
```

其中函数参数中 `char <-` 表示这个 channel 是只写频道，不能用来读取数据。

```go
type Server struct {
    mu sync.Mutex
    sub map[char<- Event]bool
}
...
func (s *Server) Subscribe(c char<- Event) {
    s.mu.Lock()
    defer s.mu.Unlock()
    if s.sub[c] {
...
func (s *Server) Cancel(c char<- Event){
    // lock....
    close(c)
    delete(s.sub, c)
}
...
### 3. 备份/可靠性服务(replicated client service)
...
type ProtocolMux interface {
    // 初始化管理不同服务的多路复用器
    Init(Service)
    // 向对应服务发送请求，多个 goroutines 并发调用这个方法
    Call(Msg) Msg
}
...
<img src="分布式系统.assets/image-20230223142029908.png" alt="image-20230223142029908" style="zoom:80%;" />

> 主要了解这篇文章中的**缓存一致性**(cache coherency)

frangipani 是一个集群分布式文件系统，使用虚拟磁盘的概念来对文件系统进行管理，并且拥有十分简单易于理解的内部架构，不同机器之间共享存储，使用锁机制来同步访问文件，并且提供系统恢复、再配置以及负载均衡等特性。

Frangipani 确保对于 Petal 虚拟磁盘上的文件，对于不同的客户端所看到的内容都是一致的，当文件发生改变的时候，客户端也能看到这一改变。

Frangipani 同样使用 WAL 预写日志的方式来简化故障恢复并且提高性能。其日志存储的方式是使用环形缓冲区的方式，当缓冲区填满的时候，首先将 25% 的日志写入到 Petal 服务器中，然后回收这 25% 的日志。

### 1. 缓存一致性

如何保持缓存一致性：

* 当对文件进读写加锁的时候，会将磁盘上的内容缓存起来。当持有写锁时候被要求降级到读锁的时候，服务器必须将脏数据写回磁盘。在没有锁的情况下，不会对文件内容进行缓存。
* 除了将脏数据写回，服务器也可以让过磁盘直接将脏数据转发给对应的读请求。（但是由于需要设计简单的设计理念，所以未实现）

防止死锁处理：首先系统决定是否需要锁，之后根据文件 inode 地址进行排序，依次获取锁。

服务器上对于文件的锁还分为状态：BUSY 状态和 IDLE 状态，处于 IDLE 状态的锁在收到锁服务器的请求的时候会首先将数据写回磁盘然后撤回锁。

### 2. 分布式事务

当需要对一系列文件进行操作的时候，事务的大致流程如下：

1. 申请所有文件的锁
2. 更新文件并且写回 Petal 服务器
3. 释放锁

如果在事务中在释放锁之前写回磁盘之后的发生崩溃，这如何解决？

* 由于使用预写式日志，只有当客户端的日志写到 petal 服务器的时候才会执行操作
* 如果崩溃之后，frangipani 会通知其他服务器重演已崩溃服务器的操作。

## 八. 分布式事务

分布式事务需要保证的四个要点：即 ACID

* Atomic，即保证整个事务是原子性的，如果中途有某个操作执行失败，那么整个事务失败
* Consistent，既保证对于不同的对象来说看到的内容是一致的
* Isolate，事务和事务之间是隔绝的，不会看到事务处理数据是的中间状态、
* Durable，当提交事务之后是可以持久化到硬盘上的。

### 1. 并发控制

Before-or-After 原子性指的是假如有两个线程 A 和 B，A 可能会在 B 的之前或者之后执行，但是不会同时执行。

并发控制主要有两种方式：

* 悲观锁并发控制，就是我们说的锁，如果其他线程正在访问临界资源，那么该线程就会阻塞等待。
* 乐观锁并发控制，不需要担心有其他事务同时对数据进行读写，但是如果同一时刻有其他事务冲突读写，需要重新进行执行事务。

两种锁的使用场景：如果经常会发生冲突会使用悲观锁的情况，在这种情况下乐观锁会出现重试事务次数较多的情况。如果冲突很少发生，使用乐观锁的执行速度会更快，使用悲观锁开销会变高。
...
使用场景：比如在一个事务中需要对于两个变量 X 和 Y 进行，但是 X 和 Y 分别存储在不同的服务器上。

两阶段提交(Two Phase Commit)流程：

* 当两个服务器都执行完毕后，事务调度器会向机器发送 Prepare 信号，机器会根据锁的状况返回 Yes / No

  ![image-20230227195448470](分布式系统.assets/image-20230227195448470.png)

* 如果回复全部为 yes 之后，事务调度器向机器正式发送 Commit 信号。收到全部机器的 Commit 成功信号之后，向客户端返回请求响应

  ![image-20230227195555158](分布式系统.assets/image-20230227195555158.png)

2PC 过程中发生崩溃怎么办？

* 当机器 A 返回 Yes 给事务调度器之前崩溃：那么该机器有权中断该事务并且忘记该事务的相关内容，返回 No。

* 返回 Yes 之后崩溃：这种情况更加复杂，所有机器都返回 Yes 给事务调度器，事务调度器会向所有机器发送 Commit 信息。此时 A 崩溃了，然而 B 机器收到 Commit 信息并且将数据持久化到硬盘上，然后释放锁。

  因此在 A 返回 Yes 之前必须要将中间数据持久化到硬盘上，当机器 A 恢复之后就可以读取持久化的中间数据并且完成事务。

崩溃处理要点：

* 如果其中一台机器已经完成 Commit 操作，就不能丢掉和忽视这个操作，不能丢掉这个事务。
* 采用 WAL 预写式日志的形式，在执行操作之前先写日志。

事务调度器等待机器响应时间过长怎么办？

* 由于机器持有锁的时间过长，其他事务也在等待这个锁。那么事务调度器可以单方面终止该事务。

但是如果机器在发送 Yes 之后很长时间未收到 Commit 信息，该机器无法单方面终止事务，因为此时可能其他机器已经收到 Commit 信息并且落盘，如果单方面终止该事务就会导致数据不一致，所以该机器只能**无限期等待**事务调度器的消息。

> 因此这里的**无限期等待**也是2PC的重要特点之一。

也可以查看这个[视频](https://www.bilibili.com/video/BV1EA411u7gM)了解。

## 九. Spanner

![image-20230228124440600](分布式系统.assets/image-20230228124440600.png)

Spanner 是一个构建在 Paxos 基础上可以切分数据的数据库，可以在不同机器之间自动切分并且转移数据，并且可以实现负载均衡以及自动故障恢复。
...
> Spanner 最重要的一点就是他们通过同步时间可以实现高效的只读事务

### 1. 具体实现

在一个地区中的一系列服务器通常称为一个 Zone。每个 Zone 都有自己独立的 ZoneMaster，管理者数以百计的 SpanServer，前者用于将数据传给 SpanServer，后者用于向客户端返回请求。其中的 location proxy 用于给客户端定位存储他们数据的服务器。

为什么要对数据进行分片：对数据分片可以对数据进行并行加速处理来提高并行吞吐量。为了应对海量的数据请求，需要将这些请求分散到数据分片所在的多个 paxos 组中，从而来做到对请求的并行处理。

<h4>0x1. Spanserver</h4>

![image-20230228125648801](分布式系统.assets/image-20230228125648801.png)

每个 Spanserver 底层维护的一个数据结构 tablet：

<center>(Key: String, timestamp:int64) -> string</center>

由于为数据添加了时间戳信息，spanner 看起来更像一个多版本控制数据库。并且使用 paxos 来实现一致性数据复制。对于日志，spanner server 分别在 paxos 和 tablet 中各保存一份，对于写请求必须由 Paxos Leader 发起，对于读请求直接发送给数据最新的 tablet。

对于 leader 来说：

* 其维护了 lock table 的数据结构用来实现并发控制，lock table 维护了两阶段锁的每个状态。
* 还维护了事务管理器用来支持分布式事务。如果对应的事务只涉及到一个 paxos 组，那么就可以忽略事务管理器直接进行操作。如果对于一个 paxos 组，需要配合两阶段提交 2PC 来进行协调完成事务。

Spanner 对于不同类型的事务采取不同的处理策略：
...
* 读写性事务：
  ...
  ![8f9f762e70b7988a1ed538fb1772b49](分布式系统.assets/8f9f762e70b7988a1ed538fb1772b49.png)

  1. 要对 x 写入首先需要对 x 进行读取，客户端会向存储 x 分片 paxos leader 发送读请求
  2. x 分片的 paxos Leader 收到信息之后对 x 进行加锁，并且返回给客户端
  3. 同样向存储 y 的 paxos leader 进行读取
  4. 加锁并且返回客户端
  5. client 选取其中一个 paxos leader 作为事务协调组，并且获取事务协调组的 id
  6. 客户端向各个 paxos leader 发送写请求，并且携带事务协调组的 id
  7. leader 将操作写入日志之后，向 follower 发送带有日志的 prepare 信息
  8. 收到大多数 follower 的 yes 之后向事务协调组也返回一个 yes 信息
  9. 当事务协调组收到所有对应分片 paxos leader 的 yes 信息之后，事务协调组向本组的其他 follower 发送 commit 信息，并且写入日志
  10. 事务协调组将 commit 信息发送给其他组的 paxos leader
  11. 其他 paxos leader 向该组的 follower 发送 commit 信息，一旦完成写操作就释放对于数据项上的锁
  12. 返回客户端的请求

* 只读事务：

  为了保证只读事务中的数据是最新的，因此在只读事务中引入了两天正确性约束：

  * [**外部一致性**]依然保证事务的执行依然是有序的，只读事务的操作可以看到之前所有事务执行的结果

  * **快照隔离技术**，对于每个事务都分配一个时间戳，读写事务的时间戳是事务提交的时候，只读事务是事务开始的时候。Spanner 中的每个事务按照时间戳的顺序来执行事务。

    Spanner 使用多版本数据的方式，在对每次写入操作的时候，保存该记录的单独副本，每个副本都和时间戳相关联。因此每次进行读请求的时候，找到该条记录比只读事务时间戳小的最大记录，那么按照时间序列就可以看到届时最新的数据。

<h4>0x2. TrueTime</h4>

![image-20230228170158354](分布式系统.assets/image-20230228170158354.png)

如何获取完美准确的时间？GPS，比如原子钟，然后通过修正传播延迟来保持时间最新。

TrueTime 有两条规则：

* 开始规则：

  r/o 操作的时间戳为事务开始时间，开始时间为 latest

  r/w 操作的时间戳为事务提交时间

* 提交准则

  在提交事务(执行完写操作并且释放锁)之前需要延迟等待一段时间。因为请求的真实事件介于 earliest 和 latest 时间之间，我们能确定的是它最晚不会超过 latest，为保证分布式环境下各个数据分片 master 的时间协调性，选择 latest 时间作为提交时间，因为每个数据分片服务器也会找时间标准 master 服务器获取时间，等到 latest 最后再提交。

  假如在 latest 之间已经完成所有操作(除了 commit)，那也不能提交，直到 latest 才能提交。
...
<img src="分布式系统.assets/image-20230224231812711.png" alt="image-20230224231812711" style="zoom:89%;" />

![image-20230226132957054](分布式系统.assets/image-20230226132957054.png)

目的：在 raft 基础上构建一套 KV 数据库

知识点：

* 在 `TestSpeed3A` 中需要完成在 1 秒之内完成 30 个日志的提交操作，然后 raft 的心跳在 100ms 左右，只能达到 1 秒 10 个日志，因此需要对 raft 的心跳机制进行优化，当没有日志到来的时候心跳间隔仍然是 100ms，当有日志的时候需要提前退出心跳等待间隔。

  我的是实现是将 100ms 的时间间隔分为 10 个 10ms 的睡眠间隔，每次唤醒的时候检查是否有新的日志到来，如果有新的日志则退出睡眠等待之间想 follower 发送新的日志来提高吞吐量。

  ```go
  times := 10
  for times > 0 {
      time.Sleep(HeartbeatInterval / 10)
      times--
      rf.mu.RLock()
...
* 为了防止出现信息丢失的情况，只有当对应日志提交的时候才会对客户端的请求做出响应。

* 为了防止出现处理重复请求的情况，对于客户端的每个请求都使用唯一ID来进行标记，在 raft 的提交的时候将对应处理过的 ID 存入缓存，当请求进来的时候先检查缓存中是否存在，如果存在则直接返回，如果不存在则继续处理。

* 当一个操作开启多个线程后，如果操作完成之后但是多个线程依然正在运行，如何优雅地告诉子线程本操作已经结束并且不会让子线程剩余操作影响系统：

  使用 `context` 包，其可以将线程和线程之间构建层级关系，关闭父级 context 即可关闭所有子 context

  ```go
  root, cancel := context.WithTimeout(context.TODO(), RPCTimeout)
  for i := range workers {
      son, cancelSon := context.WithCancel(root)
  ...
  case res := <-ch:
      // do something
      cancel() // exit all sub goroutines
      lock.Lock()
      close(ch)
      lock.Unlock()
      return res
...
* 当客户端在发送给服务器的过程中，很可能会发生服务器在提交日志之后返回客户端之前发送崩溃，因此需要额外的信息触发机制来检测这一故障，否则会发生超长时间持有锁的情况导致服务器业务停摆。在检测的时候可以检测服务器的角色和任期是否发生改变，如果发生改变则之间返回客户端错误，及时释放锁。

<h4>0x2 Lab3B</h4>

![image-20230227224808877](分布式系统.assets/image-20230227224808877.png)

![image-20230228101142464](分布式系统.assets/image-20230228101142464.png)
