...
* InnoDB 引擎在 MySQL 5.7 之前自增值保存在内存并未持久化，每次重启找 max(id)，然而取 max(id)+1 作为当前自增值，MySQL 8.0 之后才会自增持久化。

自增值修改机制：

1. 如果插入数据 id 字段为 0、null或者未指定值的时候就把当前的 AUTO_INCREMENT 填到自增字段；
2. 如果插入数据时 id 字段指定值为 X，就使用指定的值。假设当前自增值为 Y，如果 X < Y，那么这个表的自增值不变，如果 X ≥ Y，则修改为新的自增值。

<img src="database.assets/image-20220621195709557.png" alt="image-20220621195709557" style="zoom:50%;" />
...
MySQL的配置是通过命令行参数以及配置文件来获取配置信息，在 unix 系统，MySQL 配置文件通常位于 `/etc/my.cnf` 或者 `/etc/mysql/my.cnf`。
