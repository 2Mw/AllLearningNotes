...
1. 基于进程的并发编程，内核会自动交错(interleaves)执行多个逻辑流，每个流程有独立的内存地址空间。
2. 基于时间的并发编程，程序员可以手动交错执行多个逻辑流，每个流程共享同一个地址空间，可以使用 IO 多路复用来进行交错调度。
3. 基于线程的并发编程，内核用于处理多个逻辑流

### 1. 基于进程的并发编程

![image-20221015133051669](csapp.assets/image-20221015133051669.png)

上图即为使用基于进程的并发服务器，当客户端连接进入的时候，fork 子进程来处理客户端连接，继续进入 accept 流程，可以在处理该客户端连接的同时继续接收其他客户端的连接。每个客户端的请求都由独立的子进程来进行处理。
...
![image-20221015144822467](csapp.assets/image-20221015144822467.png)
...
![image-20221015150925295](csapp.assets/image-20221015150925295.png)

![image-20221015151009569](csapp.assets/image-20221015151009569.png)

线程切换相比于进程切换开销更小，线程切换只需要改变 program context，不会改变页表、虚拟地址空间以及其他线程的上下文。
...
C 线程 API ——`pthread_detach`：当一个线程被 detach 之后，该线程资源将会由内核自动释放，无需其他线程 `join` 来回收资源，也无法变成 joinable 方式。
...
![image-20221016164206661](csapp.assets/image-20221016164206661.png)
...
* 偏向读者的读者-写者模型：读者的优先级较高，写者可能会出现饥饿现象
  ...
  ![image-20221016180246387](csapp.assets/image-20221016180246387.png)
...
2. 在不同函数调用之间以来其他函数中的持久状态变量
   ...
   ![image-20221016184459634](csapp.assets/image-20221016184459634.png)
